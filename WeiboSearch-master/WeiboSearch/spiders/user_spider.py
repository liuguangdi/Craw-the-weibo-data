...
from .urlList import URL_TWEET_LIST, USER_LIST
from ..items import *
import datetime
import re
...
class UserSpider(scrapy.Spider):
    ...
    url_format = "https://weibo.cn/{}?"

    def start_requests(self):
        for user in USER_LIST:
            url_curr = self.url_format.format(user)
            print('!!!Start:' + url_curr)
            yield Request(url_curr, callback=self.parse_tweet, dont_filter=True, errback=self.Errshow)
    ...
    def parse_tweet(self, response):
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                if(tweet_node.xpath('.//span[@class = "kt" and contains(text(), "置顶")]')):
                    continue
                ...
                if "来自" in create_time_info:
                    # 微博发表时间
                    tweet_item['created_at'] = parse_time(create_time_info.split('来自')[0].strip())
                    # 发布微博的工具
                    # tweet_item['tool'] = create_time_info.split('来自')[1].strip()
                else:
                    tweet_item['created_at'] = parse_time(create_time_info.strip())

                #只要区间内的微博
                if(tweet_item['created_at'] > DATE_END):
                    print('continue!!:' + tweet_item['created_at'])
                    continue
                elif(tweet_item['created_at'] < DATE_START):
                    print('return!!:' + tweet_item['created_at'])
                    return
                ...
                if all_content_link:
                    ...
                    yield Request(all_content_url, callback=self.parse_all_content, meta={'item': tweet_item},dont_filter=True
                                  )
                else:
                    # 微博内容
                    tweet_item['content'] = \
